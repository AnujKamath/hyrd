...
from .utils import extract_resume, extract_job_description, ResumeMatcher
...
def company_list(request):
    ...
    return render(
        request, "employer/company_list.html", {"companies": companies, "user": user}
    )
...
def company_create(request):
    ...
    return render(request, "employer/company_form.html", {"form": form, "user": user})
...
def job_create(request, company_id):
    ...
    return render(
        ...
        {"form": form, "company": company, "user": user},
    )
...
def job_detail(request, job_id):
    ...
    job_data = {
        ...
        "Requirements": {
            ...
            "Experience (Years of experience)": job.experience_years
        },
    ...
    for applied_candidate in applied_candidates:
        candidate = applied_candidate.candidate
        
        # Assuming candidate resumes are stored as file paths in `candidate.resume_file`
        resume_path = candidate.resume_file.path if candidate.resume_file else None
        
        if resume_path:
            try:
                resume_data = extract_resume(resume_path)
                match_result = matcher.match_job_resume(job_data, resume_data)
                ...
                print(f"Error processing resume for {candidate.name}: {e}")
    ...
    if request.method == "POST":
        ...
        if selected_candidates:
            ...
            AppliedJob.objects.filter(
                job=job, candidate_id__in=selected_candidates
...
def job_autofill(request, company_id):
    ...
    try:
        ...
        fields = {
            ...
            "educational_requirements": job_data.get("requirements", {}).get(
                "educational", ""
            ),
            "technical_requirements": job_data.get("requirements", {}).get(
                "technical", ""
            ),
            "preferred_qualifications": "\n".join(
                job_data.get("preferred_qualifications", [])
                if isinstance(job_data.get("preferred_qualifications"), list)
                else []
            ),
